    ...
    , is_long_(size_ > SSO_BUFFER_SIZE)
{
    if (is_long_) {
        ...
        std::memcpy(long_string_.data, str, size_ + 1);
        long_string_.capacity = size_;
    } else {
        // Copy to SSO buffer
        std::memcpy(sso_buffer_, str, size_ + 1);
    }
}
...
String::String(const String& other)
    ...
    if (is_long_) {
        // Copy in heap buffer
        long_string_.data = new char[size_ + 1];
        std::memcpy(long_string_.data, other.long_string_.data, size_ + 1);
        long_string_.capacity = other.long_string_.capacity;
    } else {
        // Copy to SSO buffer
        std::memcpy(sso_buffer_, other.sso_buffer_, size_ + 1);
    }
}
